   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         sleep_until( ini_sec );
				...
				time_point<steady_clock> tiempo_real = steady_clock::now();			
				steady_clock::duration duracion = tiempo_real - ini_sec;
				
				cout << "Fin de la iteración del ciclo secundario con retraso de: " << milliseconds_f(duracion).count() << " ms." << endl;
				
				
				if ( milliseconds_f(duracion) > milliseconds_f(20)  ) {
					cout << "Error: Ha excedido el cupo maximo en esta iteracion." << endl;
					exit(-1);
				}
